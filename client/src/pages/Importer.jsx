...
const Importer = () => {
    ...
    const fetchSavedMappings = async () => {
        try {
            const res = await api.get(`/import/${selectedConnection}/mappings/${selectedTable}`);
    ...
    const confirmSaveMapping = async () => {
        ...
        try {
            await api.post(`/import/${selectedConnection}/mappings`, {
    ...
    const fetchColumns = async (connId, table) => {
        try {
            const res = await api.get(`/explorer/${connId}/columns/${table}`);
    ...
    const handleFileUpload = async () => {
        ...
        try {
            ...
            // Initialize mappings (auto-match if same name)
            const initialMappings = {};
            res.data.headers.forEach(h => {
                initialMappings[h] = ''; // User must select
            });
    ...
    const handleSheetChange = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await api.post(`/import/${selectedConnection}/sheet-preview`, {
                fileId,
                sheetName: newSheet
            });
            setPreview({ headers: res.data.headers, rows: res.data.preview });
    ...
    const handleImport = async () => {
        if (!selectedTable) return alert('Select target table');
        ...
        setLoading(true);
        try {
            const res = await api.post(`/import/${selectedConnection}/execute-import`, {
                table: selectedTable,
